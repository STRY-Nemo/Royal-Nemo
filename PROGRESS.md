...
- **Themes and glass cards** (`src/ui/theme.ts`, `src/ui/ThemeSheet.tsx`, `[data-theme]` palettes in `tokens.css`): per-device choice in localStorage `stry-theme`, applied before first paint in `main.tsx`; `Backdrop` loads `art/themes/<id>.jpg` and falls back to `art/app-background.jpg`. Cards are 52% card colour + blur so the background shows. Tab bar grid is 5 columns (the Ideas tab was wrapping off-screen at 4).

...
- **Artwork slots**: any of the files listed in `docs/ART_PROMPTS.md` dropped into `public/art/` (banners for Home, Canyon, Organize, Members and Login, team emblems, the bear den backdrop, empty-state illustrations) appear automatically; nothing renders until the file exists.
- **STRY Bear**: an alliance mascot everyone feeds by tapping (Home card and the den at `#/bear`). Every tap gets a different little reaction (jump, wiggle, spin, squish, clap, dance, nod, flip) with floating emoji and a quip; movement drops out under Reduced motion. The "toot" reaction plays `public/sfx/fart.mp3` when present (a synthesized stand-in otherwise); Settings has a per-phone "Bear sound effects" switch. One feed per 2 s per person, 60 a day, 15 evolution stages from Newborn Cub to the armored STRY Bear, shared progress stored on the server (`POST /mascot/feed`). Drop real artwork into `public/bear/stage-01.png` … `stage-15.png` (prompt in `docs/BEAR_ART_PROMPT.md`); a drawn placeholder is used until then.
- **Themes**: a palette button on Home (and Settings → Theme) switches the full-screen background and colour palette per device: Canyon Night plus the ten STRY wallpapers (Rose Nebula, Jedi Sanctuary, Sith Eclipse, Leviathan Depths, Solar Phoenix, Frost Crown, Neon Ronin, Emerald Dream, Astral Dunes, Event Horizon) in `public/art/themes/` (`docs/ART_PROMPTS.md` explains how to add more). Cards are glass (see-through with blur) so the art shows through.
- **Celestial flare**: a starfield canvas with twinkle, drift and occasional shooting stars (static under Reduced motion, hidden when Off), a glowing brand mark and gradient-lit raised cards.
- **Guest tour**: `#/guest` (Settings → "Show the app to others" shares it; the sign-in screen offers "Explore as a guest") runs the app in demo mode with sample data and the leader view, so anyone can look around without an account and without seeing live alliance data. "Exit tour" returns to sign-in.
- **One-link invites**: leaders share a single join link (`#/join/CODE`); it opens a three-step join form (in-game name → suggested username → PIN). Invalid, used-up or expired links say so before anything is typed (`GET /invites/:code/check`). A roster name that someone already joined as is shown greyed with "Already joined · sign in instead" and refused by the server (`member_claimed`), so nobody can claim a name twice. A separate **Leader join link** (Settings → Alliance accounts) creates leader accounts for everyone who opens it: you choose how many seats (2–50), it expires in 7 days, and it can be revoked at any time; the join screen says "You're invited as a leader". Leaders can **Reset PIN** for a member who forgot theirs (Settings → Alliance accounts → the account); it signs that account out everywhere.
...
- **Ideas tab**: any member posts a suggestion or feature request (title + details, up to 5 open per person), votes on others' ideas, and sees leader replies. Leaders set a status (New → Planned → Done / Declined) with an optional reply. Every idea keeps an activity trail (who posted it, who set each status, who replied and what they said), shown under the card and used to name the replier on the reply. A scheduled workflow (`.github/workflows/suggestions-sync.yml`, every 6 h) mirrors new ideas into GitHub issues labelled `idea` and keeps a top-voted digest issue, so a coding agent can review and analyse them. The only setup is the repository secret `SUGGESTIONS_SYNC_TOKEN`; the workflow copies it onto the Worker itself on first run.
- **In-game team screen import** (Canyon → "Import the in-game team screen", leaders): reads the recording export (.xlsx/.csv), matches names to the roster (case, accents and leader name mappings), shows a review (starters, substitutes, declined, other team, unmatched names, availability updates), then writes that team's starters and substitutes exactly as the game shows them, unlocked, plus the availability those rows imply. Leaders can move, swap or lock players right up to the match; Regenerate asks before replacing a lineup and keeps any locks. It records selection only; attendance is still confirmed after the match. The same rules run on the server.

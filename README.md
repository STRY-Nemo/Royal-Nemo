...
- **One-link invites**: leaders share a single join link (`#/join/CODE`); it opens a three-step join form (in-game name → suggested username → PIN). Invalid, used-up or expired links say so before anything is typed (`GET /invites/:code/check`).

...
- **Themes**: a palette button on Home (and Settings → Theme) switches the full-screen background and colour palette per device: Canyon Night, Dawn, Storm, Ember, Frost, Void. Each theme reads `public/art/themes/<id>.jpg` and falls back to the canyon sky until that file exists (`docs/ART_PROMPTS.md`). Cards are glass (see-through with blur) so the art shows through.
